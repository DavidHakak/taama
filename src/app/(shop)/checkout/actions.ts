...
import { db } from '@/db'
import { shopOrders, shopOrderItems, shopProducts, profiles, shopCoupons, shopProductVariants, shopEvents, storeSettings } from '@/db/schema'
    ...
    const newOrderId = await db.transaction(async (tx) => {
      ...
      if (couponCode) {
        ...
        if (subtotal < minOrder) {
          throw new Error(`קוד קופון זה דורש סכום הזמנה מינימלי של ₪${minOrder.toFixed(2)}`)
        }

        const val = Number(coupon.discount_value)
        if (coupon.discount_type === 'percentage') {
          couponDiscountVal = (subtotal * val) / 100
        } else {
          couponDiscountVal = val
        }

        couponDiscountVal = Math.min(subtotal, couponDiscountVal)
        couponId = coupon.id
